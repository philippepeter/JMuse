...
public class Key {
	...
	public void draw(int x, int y, Graphics2D g) {
		...
		if(note.getKey() == 0 || note.getKey() == 5) {
			int[] xPoints = {x, x, x+blackWidth,x+blackWidth,x+WIDTH,x+WIDTH};
			int[] yPoints = {y,y-HEIGHT, y-HEIGHT,y-HEIGHT+blackHeight,y-HEIGHT+blackHeight,y};
			g.setColor(wcolor);
		...
		} else if(note.getKey() == 1 || note.getKey() == 6 || note.getKey() == 8) {
			int[] xPoints = {x + blackWidth, x + blackWidth , x + 2*blackWidth ,x + 2*blackWidth};
			int[] yPoints = {y-HEIGHT+blackHeight,y-HEIGHT,y-HEIGHT,y-HEIGHT+blackHeight};
			g.setColor(bcolor);
			...
			int[] xPoints = {x + WIDTH - littleDelta , x + WIDTH - littleDelta, x + WIDTH-littleDelta+blackWidth,x+WIDTH - littleDelta+blackWidth};
			int[] yPoints = {y-HEIGHT+blackHeight,y-HEIGHT,y-HEIGHT,y-HEIGHT+blackHeight};
			g.setColor(bcolor);
			shape = new Polygon(xPoints, yPoints,4);
			g.fill(shape);
		} else if(note.getKey() == 4 || note.getKey() == 11) {
			int[] xPoints = {x, x, x+blackWidth-littleDelta,x+blackWidth-littleDelta,x+WIDTH,x+WIDTH};
			int[] yPoints = {y,y-HEIGHT+blackHeight,y-HEIGHT+blackHeight,y-HEIGHT,y-HEIGHT,y};
			g.setColor(wcolor);
			shape = new Polygon(xPoints, yPoints,6);
			g.fill(shape);
			g.setColor(bcolor);
...
//	}
	...
	private void drawCircle(Color color, int x, int y, Graphics2D g, boolean fill) {
		if(note.isSharp()) {
			...
			double radius = 3;
			Shape circle = new Ellipse2D.Double(centerx-radius, centery-radius,2*radius, 2*radius);
			g.setColor(color);
			if(fill) {
				g.setColor(color);
				g.fill(circle);
